...
var SourceMenuItem = function (_MenuItem) {
  ...
  // this is necessary for the css styling. it adds a class to the selected quality level and 
  // removes the class from previously selected quality level (if necessary)
  // this part is written by Samia Ruponti, for Bongo Solutions LTD and Bioscope

  SourceMenuItem.prototype.sibilings = function sibilings(element) {
    // get next sibiling elements
    function nextSibiling() {
      var n = element;
      var r = [];
    ...
    // get prev sibiling elements
    function prevSibiling() {
      var n = element;
      var r = [];
    ...
    // concat both to get all sibilings

    var prev = prevSibiling() || [],
        next = nextSibiling() || [];
  ...
  SourceMenuItem.prototype.handleClick = function handleClick() {
    ...
    // add the CSS class to the selected level

    var allSibilings = this.sibilings(this.el_);

    for (var classes = 0; classes < allSibilings.length; classes++) {
      allSibilings[classes].classList.remove('selected-quality');
    }
...
var SourceMenuButton = function (_MenuButton) {
  ...
  SourceMenuButton.prototype.createItems = function createItems() {
    ...
    if (this.player().qualityLevels().length > 1) {
      var autoOption = new SourceMenuItem(this.player_, {
        label: 'Auto',
        index: this.player().qualityLevels().length,
        selected: false
      });

      //"auto" should be checked as default quality level.
      autoOption.el_.classList.add('selected-quality');
      menuItems.push(autoOption);
    }
    return menuItems;
  };
