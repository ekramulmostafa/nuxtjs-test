...
(function (global, factory) {
	...
	(global.videojsSendcustomerfeedback = factory(global.videojs));
...
var getPlayerErrors = function getPlayerErrors(player) {
...
var messageContainer = _createElement('div', 'message');
...
function sendData(form, url, userIp, modal) {
  ...
  var _element = document.getElementsByClassName('vjs-feedback-container');
  var loader = _element[0].getElementsByClassName('loader')[0];

  var XHR = new XMLHttpRequest();
  var feedbackFormData = new FormData(form);
  ...
  XHR.addEventListener('load', function (event) {
    ...
    if (XHR.status < 400) {
      ...
      // shows error message
        ...
        var errorMessage = 'Sorry! There was a problem and your feedback could not be submitted. Perhaps try again later?';

        //we have to handle specific error code. 403 is very important

       /* if (XHR.status === 403) {

            return errorMessage = 'You need to login to submit the feedback.';

        }*/

        // shows error message
        console.log('something');
        messageContainer.innerHTML = '<div>' + '<h4>' + errorMessage + '</h4>' + '<p>' + 'Error: ' + event.target.status + '&nbsp;' + event.target.statusText + '</p>' + '</div>';
...
var constructFeedbackOptions = function constructFeedbackOptions(player, options) {
  ...
  var feedback = options.feedbackOptions;

  var _frag = document.createDocumentFragment();

  var container = _createElement('div', 'vjs-feedback'),
      header = _createElement('div', 'vjs-feedback-header'),
      title = _createElement('h3', 'vjs-feedback-form-title'),
      description$$1 = _createElement('p', 'vjs-feedback-form-description'),
      _form = _createElement('form', 'vjs-feedback-form');

  _form.name = 'videojs-feedback';

  title.innerHTML = options.title;
  description$$1.innerHTML = options.description;

  header.appendChild(title);
  header.appendChild(description$$1);

  // here are the checkboxes
  var j = feedback.length - 1;
  for (var i = 0; i <= j; i++) {

    var _div = _createElement('div', 'checkbox'),
    ...
    var _text = _createElement('h5');
    _text.innerHTML = feedback[i].text;

    var _subtext = _createElement('h6');
  ...
  var button = _createElement('button', 'form-submit');
  button.type = "button";
  button.innerHTML = "send feedback";

  var message = _createElement('div', 'error-message');
  ...
  var contentEl = _createElement('div', 'vjs-feedback-container');
  ...
  var floatingButton = _createElement('button', 'open-feedback-form');
  ...
  button.addEventListener('click', function (event) {
    ...
    if (!checkEmptyForm()) {
      ...
      var loader = _createElement('div', 'loader');
...
var onPlayerReady = function onPlayerReady(player, options) {
  ...
  player.on('ready', function () {
    return constructFeedbackOptions(player, options);
  });
};
 ...
 */
var sendcustomerfeedback = function sendcustomerfeedback(options) {
  var _this = this;
